...
void solve(ofstream &fin, ofstream &fout)
{
	int t = rng() % 10 + 1;
	fin << t << endl;
...
signed main()
{
	...
	for (int i = 0; i < 10; i++)
	{
		// make ipop folder beforehand
		ofstream fin;
		string zero_padd = "";
		if (i < 10) zero_padd += "0";
		string ifile = "ipop/bigtc" + zero_padd + to_string(i) + ".in";
		fin.open(ifile);
		ofstream fout;
		string ofile = "ipop/bigtc" + zero_padd + to_string(i) + ".out";
		fout.open(ofile);
		solve(fin, fout);
		fin.close();
		fout.close();
